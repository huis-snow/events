...
- [눈치 숫자](./nunchi-number/) — 참가 인원에 맞는 범위에서 숫자를 몰래 고른 뒤, 중복되지 않은 가장 작은 숫자를 고른 사람이 선택한 규칙대로 점수를 얻는 실시간 심리전
...
- 같은 숫자를 고른 사람은 모두 탈락하고, 남은 숫자 중 가장 작은 숫자를 고른 한 명이 점수를 얻습니다.
- 기본 `구간 보상`은 선택 범위를 세 구간으로 나눠 높은 구간의 승리 숫자에 1·2·3점을 줍니다. 방 생성 시 승리 숫자를 그대로 점수로 받는 `숫자 보상`, 항상 1점인 `클래식`도 선택할 수 있습니다.
