...
test("이벤트 게임 주소에는 세션·경기·게임 방 정보가 모두 포함된다", () => {
  ...
  assert.equal(
    core.gameUrl("pushluck", "ABCDEFGH", "M007", "23456789"),
    "./one-more-step/?event=ABCDEFGH&match=M007&room=23456789",
  );
});
...
test("이벤트 포탈과 여섯 게임은 저음량 배경 음악을 공유 설정으로 재생한다", () => {
  ...
  const scripts = ["app.js", "bingo/app.js", "nunchi-number/app.js", "chosung-escape/app.js", "minority-survival/app.js", "now-timing/app.js", "one-more-step/app.js"]
    .map((relativePath) => fs.readFileSync(path.join(repositoryRoot, relativePath), "utf8"))
    .join("\n");
